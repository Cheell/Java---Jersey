...
public class CouponController {
	...
	/*	if(coupon == null) {
			throw new ApplicationException(ErrorType.NOT_FOUND_IN_DB, DateUtils.getCurrentDateAndTime() + 
					" Get coupon by id has failed."
					+ "\nSearch for coupon Id  that doesn't exist.");
		}
	*/
		return coupon;
	}
	...
	/*	if(coupon == null) {
			throw new ApplicationException(ErrorType.NOT_FOUND_IN_DB, DateUtils.getCurrentDateAndTime() + 
					" Get coupon by title has failed."
					+ "\nSearch for coupon title that doesn't exist.");
		}
	*/
		return coupon;
	}
	...
	/**
	 * Search for coupons with particular company name.
	 * @param name company name to search for.
	 * @return list of coupons that were found.
	 * @throws ApplicationException
	 */
	public List<Coupon> getCouponsByCompanyId(String name) throws ApplicationException {
		return couponDao.getCouponsByCompanyName(name);
	}
	...
	public List<CouponPurchase> getCouponPurchasesByCustomerId(long customerId) throws ApplicationException {
		...
		if(customerDao.getCustomerByCustomerId(customerId) == null) {
			throw new ApplicationException(ErrorType.NOT_FOUND, DateUtils.getCurrentDateAndTime() + 
				"get coupons by customer id failed." + 
				"\nCustomer with such an id doesnt exist.");
		}
		return couponDao.getCouponPurchasesByCustomerId(customerId);
	}
	 ...
	 * @throws ApplicationException
	 */
	public void makePurchase(CouponPurchase couponPurchase) throws ApplicationException {
		//coupon id check
		if(couponDao.getCouponByCouponId(couponPurchase.getCouponId()) == null) {
			throw new ApplicationException(ErrorType.NOT_FOUND, DateUtils.getCurrentDateAndTime() + 
		...
		int newCouponAmount = currentCouponAmount - couponPurchase.getAmount();
		if( newCouponAmount <= 0) {
			throw new ApplicationException(ErrorType.INCORRECT_VALUE, DateUtils.getCurrentDateAndTime() + 
					"Purchase coupon failed." + 
					"\nAttempt to purchase more coupons than we have in DB.");
		}
		couponDao.addCustomerCoupon(couponPurchase.getCouponId(), couponPurchase.getCustomerId(), couponPurchase.getAmount());
		couponDao.updateCouponAmmount(couponPurchase.getCouponId(), newCouponAmount);
	}

	public List<Coupon> getCouponsByCompanyName(String name) throws ApplicationException {		// TODO Auto-generated method stub
		return this.couponDao.getCouponsByCompanyName(name);
	}
}